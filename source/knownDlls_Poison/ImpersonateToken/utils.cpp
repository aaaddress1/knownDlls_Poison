...
VOID PrintDebug(LPCWSTR pwszFormat, ...)
{
	if (g_bDebug)
	{
		...
		dwDebugStringLen += _scwprintf(L"[%ws] ", wszUsername) * sizeof(WCHAR);
		dwDebugStringLen += _vscwprintf(pwszFormat, va) * sizeof(WCHAR) + 2;
		pwszDebugString = (LPWSTR)LocalAlloc(LPTR, dwDebugStringLen);

		if (pwszDebugString)
		{
			StringCbPrintf(pwszDebugString, dwDebugStringLen, L"[%ws] ", wszUsername);
...
HANDLE ObjectManagerCreateDirectory(LPCWSTR dirname)
{
	...
	status = NtCreateDirectoryObject(&hDirectory, DIRECTORY_ALL_ACCESS, &oa);
